...
run_params = {'verbose':True,
              ...
              'total_mass_init': 5e6,
              # SPS parameters
              #'agelims': [0., 8.0, 8.3, 8.5, 8.8, 9.1, 9.4, 9.7, 10.14],
...
def load_model(agelims=[], mock_params=['logzsol', 'dust2'],
               total_mass_init=3e10, fit_rv=False, fit_tesc=False,
    ...
    model_params[mind]['prior_args'] = {'maxi':mass_init.max()*10000.0,
                                                   'mini':mass_init.min() * 0.0001}
    ...
    if fit_tesc:
        model_params[pnames.index('dust1')]['isfree'] = True
        if fit_rv:
            model_params[pnames.index('dust1_index')]['isfree'] = True
        model_params[pnames.index('dust2')]['isfree'] = False
        model_params[pnames.index('dust2')]['init'] = 0.0
        model_params[pnames.index('dust_tesc')]['init'] = agebins[0,1]
